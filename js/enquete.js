...
(function () {
 ...
 /* --- Departements ----------------------------------------------------------
    En saisie libre, la meme reponse arrive sous quatre formes (« Ile-de-France »,
    « 78 », « Yvelines », « idf ») et rien ne s'agrege. Une liste fermee, groupee
    par region, donne le departement ET la region en un seul controle, et le
    telephone ouvre son selecteur natif plutot qu'un clavier.
    Les collectivites d'outre-mer y figurent : la federation y a des ligues. */
 const DEPARTEMENTS = [
  ...
  ['Outre-mer', ['971 Guadeloupe','972 Martinique','973 Guyane','974 La Réunion','975 Saint-Pierre-et-Miquelon','976 Mayotte','977 Saint-Barthélemy','978 Saint-Martin','986 Wallis-et-Futuna','987 Polynésie française','988 Nouvelle-Calédonie']],
  ['Hors de France', ['Hors de France']]
 ];

 /* La region se deduit du departement choisi : autant l'enregistrer, une etude
    territoriale se lit aussi bien a l'echelle de la ligue qu'a celle du
    departement, et cela ne coute pas une question de plus au repondant. */
 const REGION_DE = {};
 DEPARTEMENTS.forEach(function (g) { g[1].forEach(function (d) { REGION_DE[d] = g[0]; }); });
 ...
 function champ(q) {
  ...
  } else if (q.type === 'departement') {
   corps = `<select id="${q.id}" name="${q.id}"${req}>
     <option value="">Choisir un département</option>
     ${DEPARTEMENTS.map(([region, deps]) => `<optgroup label="${region}">${
       deps.map((d) => `<option${v === d ? ' selected' : ''}>${d}</option>`).join('')
     }</optgroup>`).join('')}
    </select>`;
 ...
 function envoyer() {
  ...
  spec.sections.forEach(function (sec) {
   sec.questions.forEach(function (q) {
    ...
    if (q.type === 'departement' && REGION_DE[v]) {
     plates.push({
      id: q.id + '_reg',
      libelle: 'Région (déduite du département)',
      type: 'liste',
      section: texteBrut(sec.titre),
      valeur: REGION_DE[v]
     });
